...
<?php
	...
	Theme::asset()->add('core-style-summernote', '//cdnjs.cloudflare.com/ajax/libs/summernote/0.8.11/summernote-bs4.css');
<<<<<<< HEAD
	Theme::asset()->add('core-js-summernote', '//cdnjs.cloudflare.com/ajax/libs/summernote/0.8.11/summernote-bs4.js');
=======
	Theme::asset()->add('core-js-summernote', '//cdnjs.cloudflare.com/ajax/libs/summernote/0.8.11/summernote-bs4.js'); 
>>>>>>> 930ca71faceb5e742ade35552da8b36d53e640a5
	...
	Theme::asset()->writeScript('loadLazy',' 
		...
		$(".addSelectSubRegion").on("change",".selectSubRegion",function() {
			...
			$("input[name=subRegionName]").val($(this).find("option:selected").text()); 
		});
		function getRegions(){
					...
					if(result.success==true){
						...
						$.each(result.subregion, function(i, item) {
							if(item._id.$oid==$("input[name=idSubRegion]").val()){
								$(".addSelectSubRegion .selectSubRegion").append("<option value="+item._id.$oid+" selected>"+item.title+"</option>");
							}else{
								$(".addSelectSubRegion .selectSubRegion").append("<option value="+item._id.$oid+">"+item.title+"</option>");
							}
						}); 
						function format(icon) {
							var originalOption = icon.element;

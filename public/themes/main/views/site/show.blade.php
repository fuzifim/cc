			...
			<div class="col-md-8">
				<div class="card form-group">
					<div class="card-body">
						...
						@endif
						<h1 class="card-title"><a href="{{route('go.to',array(config('app.url'),$note->id,str_slug($note->title, '-')))}}" rel="nofollow" target="blank">{{str_limit(strip_tags(html_entity_decode($note->title),''), $limit = 100, $end = '...')}}</a></h1> 
...
<?
	...
	Theme::asset()->writeScript('loadLazy',' 
		...
		$(".statusDomain").click(function(){
			...
			$.ajax({
				url: "https://'.config('app.url').'/site/'.$note->id.'",
